...
DSN = 'postgresql://login:password@localhost:5432/VKinder'  # Определяем параметры подключения к базе данных
...
users = Table('users', metadata,  # Создаем таблицу пользователя
              ...
              Column('vk_id', String(20), nullable=False),  # Создаем поле vk_user с типом данных String
...
def get_user_data(vk_id):  # select запрос в таблицу users
    select_query = Select(users.c.id, users.c.gender, users.c.age, users.c.city
        ...
        users.c.vk_id == str(vk_id))  # Указываем параметры select запроса, что достать и условие поиска
...
def get_pair_data(vk_id):  # select запрос в таблицу pair
...
def get_favorite_data(vk_id):  # select запрос в таблицу favorite
    selection_query = Select(
        pair.c.first_name, pair.c.last_name, pair.c.profile_url, pair.c.photos
                  ...
                  ).where(users.c.vk_id == str(vk_id))  # Определяем параметры запроса
